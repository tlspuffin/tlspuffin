    ...
    PT: ProtocolTypes + 'static,
{
        ...
        PB: ProtocolBehavior<ProtocolTypes = PT> + 'static,
    {
        ...
        */
        ...
        // (FixedStackMutator in stages.rs is available to force an exact stack size for
        // experiments.) Wrap the geometric stacking mutator so that, when enabled
        // (default), every anchor picked by the mutations stacked in one stage is confined
        // to a single random step -- avoiding the "coverage hitchhiker" where one stacked
        // edit gains coverage while another breaks a later step yet the broken-tailed trace
        // is still saved. Disable with `--no-step-lock`.
