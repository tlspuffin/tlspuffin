...
pub trait VariableData<PT: ProtocolTypes>: Debug + ExtractKnowledge<PT> {
    fn boxed(&self) -> Box<dyn VariableData<PT>>;
    fn boxed_any(&self) -> Box<dyn Any>;
    fn boxed_extractable(&self) -> Box<dyn ExtractKnowledge<PT>>;
...
where
    T: Clone + Debug + ExtractKnowledge<PT>,
{
    fn boxed(&self) -> Box<dyn VariableData<PT>> {
        Box::new(self.clone())
    }
