...
use crate::puts::opcua_sys::{
    version_of, RustLogFilter, AGENT, AGENT_INTERFACE, ALICE_CERTIFICATE, ALICE_PRIVATE_KEY,
    APPLICATION_DESCRIPTOR, BOB_CERTIFICATE, BOB_PRIVATE_KEY, OPCUA_AGENT_ROLE,
    OPCUA_PUT_INTERFACE, VOID_STORE,
};
...
impl Stream<OpcuaProtocolBehavior> for Agent {
        ...
        if let Some(c_add_inbound) = self.c_agent_interface.add_inbound {
            unsafe {
                ...
                if let Err(cerror) = *result {
                    log::error!(
                        "Open62541: error while trying to add bytes: {}",
                        cerror.reason
                    )
                }
                if message.len() != written {
                    log::error!(
                        "Added to inbound only {} bytes out of {}!",
                        written,
                        message.len() - 1
                    )
                } else {
    ...
    fn take_message_from_outbound(
        ...
        if let Some(c_take_outbound) = self.c_agent_interface.take_outbound {
            ...
            Err(Error::Put(
                "Open62541 PUT: take_outbound unavailable!".to_string(),
            ))
        }
    }
}
...
impl Put<OpcuaProtocolBehavior> for Agent {
    fn progress(&mut self) -> Result<(), Error> {
        if let Some(agent_progress) = self.c_agent_interface.progress {
            ...
            Err(Error::Put(
                "Open62541 Agent: progress unavailable!".to_string(),
            ))
        }
    }
    ...
    fn reset(&mut self, _new_name: AgentName) -> Result<(), Error> {
        Err(Error::Put(
            "Open62541 Agent: reset unavailable!".to_string(),
        ))
    }
    ...
    fn shutdown(&mut self) -> String {
        format!(
            "Open62541 agent: {} is shutting down",
...
impl Factory<OpcuaProtocolBehavior> for Open62541Factory {
    ...
    fn create(
        ...
        unsafe {
            ...
            if let Some(create_agent) = self.put_interface.create {
                ...
                Err(Error::Put(
                    "Open62541 Factory: create Agent unavailable!".to_string(),
                ))
            }
        }
    }
    ...
    fn name(&self) -> String {
        String::from(OPEN62541)
    }

    fn versions(&self) -> Vec<(String, String)> {
        let put_version = if let Some(get_put_version) = self.put_interface.version {
