...
// with an OPC UA application instantiated in the library s2opc.
#[derive(Debug)]
struct Agent {
...
impl Factory<OpcuaProtocolBehavior> for S2opcFactory {
    ...
    fn name(&self) -> String {String::from(S2OPC)}
...
pub fn new_opcua_factory() -> Box<dyn Factory<OpcuaProtocolBehavior>> {
    unsafe {
        Box::new(S2opcFactory {
            put_interface: raw::s2opc(),
        })
    }
}
