...
static void TCP_checkStopped(UA_PuffinConnectionManager *pcm)
{
    if (pcm->fdsSize == 0 && pcm->cm.eventSource.state == UA_EVENTSOURCESTATE_STOPPING)
    {
...
static void TCP_delayedClose(void *application, void *context)
{
    ...
    UA_LOG_TRACE(el->eventLoop.logger,
    ...
    conn->applicationCB(cm,
                        (uintptr_t)conn->rfd.fd,
                        conn->application,
                        &conn->context,
...
static void TCP_shutdown(UA_ConnectionManager *cm, TCP_FD *conn);

static UA_StatusCode TCP_shutdownConnection(UA_ConnectionManager *cm, uintptr_t connectionId);

/* Gets called when a connection socket opens, receives data or closes */
static void TCP_connectionSocketCallback(UA_ConnectionManager *cm, TCP_FD *conn, short event)
{
    UA_EventLoopPuffin *el = (UA_EventLoopPuffin *)cm->eventSource.eventLoop;
    ...
    if (is_close_message)
    {
        UA_LOG_DEBUG(el->eventLoop.logger,
                     UA_LOGCATEGORY_NETWORK,
                     "TCP %u\t| Close connexion, %lu bytes",
                     conn->rfd.fd,
                     pcm->rxBuffer_index);
...
static void TCP_listenSocketCallback(UA_ConnectionManager *cm, TCP_FD *conn, UA_FD newsockfd);
...
void TCP_PuffinConnectionCallback(UA_PuffinConnectionManager *pcm)
{
    ...
    if (!conn)
    {
        ...
        if (!conn)
        {
            UA_LOG_ERROR(el->eventLoop.logger,
                         UA_LOGCATEGORY_NETWORK,
                         "TCP %u\t| Opening of connection failed",
...
static void TCP_listenSocketCallback(UA_ConnectionManager *cm, TCP_FD *conn, UA_FD newsockfd)
{
    ...
    UA_LOG_INFO(cm->eventSource.eventLoop->logger,
                UA_LOGCATEGORY_NETWORK,
                "TCP %u\t| Connection opened from \"%s\" via the listen socket TCP %u",
                (unsigned)newsockfd,
                hoststr,
                (unsigned)conn->rfd.fd);
...
static UA_StatusCode Puffin_sendWithConnection(UA_ConnectionManager *cm,
...
static UA_StatusCode
TCP_openPassiveConnection(UA_PuffinConnectionManager *pcm,
                          const UA_KeyValueMap *params,
                          void *application,
                          void *context,
                          UA_ConnectionManager_connectionCallback connectionCallback,
                          UA_Boolean validate)
{
    ...
    const UA_UInt16 *port =
        (const UA_UInt16 *)UA_KeyValueMap_getScalar(params,
                                                    tcpConnectionParams[TCP_PARAMINDEX_PORT].name,
    ...
    if (TCP_registerListenSocket(pcm,
                                 hostname,
                                 *port,
                                 application,
                                 context,
                                 connectionCallback,
                                 validate) == UA_STATUSCODE_GOOD)
    {
        retval = UA_STATUSCODE_GOOD;
    };
...
static UA_StatusCode
TCP_openActiveConnection(UA_PuffinConnectionManager *pcm,
                         const UA_KeyValueMap *params,
                         void *application,
                         void *context,
                         UA_ConnectionManager_connectionCallback connectionCallback,
                         UA_Boolean validate)
{
    ...
    const UA_String *addr =
        (const UA_String *)UA_KeyValueMap_getScalar(params,
                                                    tcpConnectionParams[TCP_PARAMINDEX_ADDR].name,
    ...
    UA_LOG_DEBUG(el->eventLoop.logger,
                 UA_LOGCATEGORY_NETWORK,
                 "TCP\t| Open a connection to \"%s\" on port %u",
                 hostname,
                 *port);
...
static UA_StatusCode TCP_openConnection(UA_ConnectionManager *cm,
                                        const UA_KeyValueMap *params,
                                        void *application,
                                        void *context,
                                        UA_ConnectionManager_connectionCallback connectionCallback)
{
    ...
    UA_StatusCode res = UA_KeyValueRestriction_validate(el->eventLoop.logger,
                                                        "TCP",
                                                        tcpConnectionParams,
                                                        TCP_PARAMETERSSIZE,
                                                        params);
    ...
    const UA_Boolean *validateParam = (const UA_Boolean *)UA_KeyValueMap_getScalar(
        params,
    ...
    const UA_Boolean *listenParam = (const UA_Boolean *)UA_KeyValueMap_getScalar(
        params,
    ...
    if (listen)
    {
        res = TCP_openPassiveConnection(pcm,
                                        params,
                                        application,
                                        context,
                                        connectionCallback,
    ...
    {
        res = TCP_openActiveConnection(pcm,
                                       params,
                                       application,
                                       context,
                                       connectionCallback,
...
static UA_StatusCode TCP_eventSourceStart(UA_ConnectionManager *cm)
{
    UA_PuffinConnectionManager *pcm = (UA_PuffinConnectionManager *)cm;
    UA_EventLoopPuffin *el = (UA_EventLoopPuffin *)cm->eventSource.eventLoop;
    if (!el)
...
static void *TCP_shutdownCB(void *application, UA_RegisteredFD *rfd)
{
...
static void TCP_eventSourceStop(UA_ConnectionManager *cm)
{
    UA_PuffinConnectionManager *pcm = (UA_PuffinConnectionManager *)cm;
    UA_EventLoopPuffin *el = (UA_EventLoopPuffin *)cm->eventSource.eventLoop;
...
static UA_StatusCode TCP_eventSourceDelete(UA_ConnectionManager *cm)
{
    UA_PuffinConnectionManager *pcm = (UA_PuffinConnectionManager *)cm;
...
UA_ConnectionManager *UA_ConnectionManager_new_POSIX_TCP(const UA_String eventSourceName)
{
    UA_PuffinConnectionManager *pcm =
        (UA_PuffinConnectionManager *)UA_calloc(1, sizeof(UA_PuffinConnectionManager));
    if (!pcm)
        return NULL;
