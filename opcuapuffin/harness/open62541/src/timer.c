...
static enum ZIP_CMP cmpDateTime(const UA_DateTime *a, const UA_DateTime *b)
{
    if (*a == *b)
        return ZIP_CMP_EQ;
...
static enum ZIP_CMP cmpId(const UA_UInt64 *a, const UA_UInt64 *b)
{
    if (*a == *b)
        return ZIP_CMP_EQ;
    return (*a < *b) ? ZIP_CMP_LESS : ZIP_CMP_MORE;
...
static UA_DateTime
calculateNextTime(UA_DateTime currentTime, UA_DateTime baseTime, UA_DateTime interval)
{
...
static void *findTimer2Batch(void *context, UA_TimerEntry *compare)
{
...
static void batchTimerEntry(UA_Timer *t, UA_TimerEntry *te)
{
                           ...
                           UA_UInt64 *callbackId)
{
    ...
    UA_DateTime nextTime =
        (baseTime == NULL) ? now + interval : calculateNextTime(now, *baseTime, interval);
...
static void *processEntryCallback(void *context, UA_TimerEntry *te)
{
...
UA_DateTime UA_Timer_process(UA_Timer *t, UA_DateTime now)
{
    ...
    UA_assert(!ZIP_MIN(UA_TimerTree, &t->tree) || ZIP_MIN(UA_TimerTree, &t->tree)->nextTime > now);
...
static void *freeEntryCallback(void *context, UA_TimerEntry *entry)
{
    UA_free(entry);
    return NULL;
}
