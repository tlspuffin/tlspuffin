...
static UA_DateTime UA_EventLoopPuffin_nextTimer(UA_EventLoop *public_el)
{
...
static UA_StatusCode UA_EventLoopPuffin_addTimer(UA_EventLoop *public_el,
                                                 UA_Callback cb,
                                                 void *application,
                                                 void *data,
...
static UA_StatusCode UA_EventLoopPuffin_modifyTimer(UA_EventLoop *public_el,
                                                    UA_UInt64 callbackId,
                                                    UA_Double interval_ms,
                                                    UA_DateTime *baseTime,
                                                    UA_TimerPolicy timerPolicy)
{
    UA_EventLoopPuffin *el = (UA_EventLoopPuffin *)public_el;
    return UA_Timer_modify(&el->timer,
                           callbackId,
                           interval_ms,
                           public_el->dateTime_nowMonotonic(public_el),
                           baseTime,
...
static void UA_EventLoopPuffin_removeTimer(UA_EventLoop *public_el, UA_UInt64 callbackId)
{
    UA_EventLoopPuffin *el = (UA_EventLoopPuffin *)public_el;
...
void UA_EventLoopPuffin_addDelayedCallback(UA_EventLoop *public_el, UA_DelayedCallback *dc)
{
    ...
    UA_DelayedCallback **oldtail =
        (UA_DelayedCallback **)UA_atomic_xchg((void **)&el->delayedTail, &dc->next);
    UA_atomic_xchg((void **)oldtail, &dc->next);
}

/* Resets the delayed queue and returns the previous head and tail */
static void resetDelayedQueue(UA_EventLoopPuffin *el,
                              UA_DelayedCallback **oldHead,
                              UA_DelayedCallback **oldTail)
{

    UA_LOG_TRACE(el->eventLoop.logger, UA_LOGCATEGORY_EVENTLOOP, "Reset delayed Queue");
    if (el->delayedHead1 <= (UA_DelayedCallback *)0x01 &&
        el->delayedHead2 <= (UA_DelayedCallback *)0x01)
    ...
    *oldHead = (UA_DelayedCallback *)UA_atomic_xchg((void **)activeHead, (void *)0x01);
    ...
    *oldTail = (UA_DelayedCallback *)UA_atomic_xchg((void **)&el->delayedTail, inactiveHead);
}

static void UA_EventLoopPuffin_removeDelayedCallback(UA_EventLoop *public_el,
                                                     UA_DelayedCallback *dc)
{
    UA_EventLoopPuffin *el = (UA_EventLoopPuffin *)public_el;
    UA_LOG_TRACE(el->eventLoop.logger, UA_LOGCATEGORY_EVENTLOOP, "Removed delayed Callback");
...
static void processDelayed(UA_EventLoopPuffin *el)
{
    UA_LOG_TRACE(el->eventLoop.logger, UA_LOGCATEGORY_EVENTLOOP, "Process delayed callbacks");
...
static UA_StatusCode UA_EventLoopPuffin_start(UA_EventLoopPuffin *el)
{
    ...
    UA_LOG_DEBUG(el->eventLoop.logger, UA_LOGCATEGORY_EVENTLOOP, "Starting the EventLoop");

    /* Setting custom clock source */
    const UA_Int32 *cs =
        (const UA_Int32 *)UA_KeyValueMap_getScalar(&el->eventLoop.params,
...
static void checkClosed(UA_EventLoopPuffin *el)
{

    UA_LOG_DEBUG(el->eventLoop.logger,
                 UA_LOGCATEGORY_EVENTLOOP,
    ...
    *(UA_EventLoopState *)(uintptr_t)&el->eventLoop.state = UA_EVENTLOOPSTATE_STOPPED;
    ...
    UA_LOG_DEBUG(el->eventLoop.logger, UA_LOGCATEGORY_EVENTLOOP, "The EventLoop has stopped");
}

static void UA_EventLoopPuffin_stop(UA_EventLoopPuffin *el)
{
    ...
    UA_LOG_DEBUG(el->eventLoop.logger, UA_LOGCATEGORY_EVENTLOOP, "Stopping the EventLoop");
    ...
    *(UA_EventLoopState *)(uintptr_t)&el->eventLoop.state = UA_EVENTLOOPSTATE_STOPPING;
    ...
    for (; es; es = es->next)
    {
        if (es->state == UA_EVENTSOURCESTATE_STARTING || es->state == UA_EVENTSOURCESTATE_STARTED)
        {
            es->stop(es);
        }
    }
...
static UA_StatusCode UA_EventLoopPuffin_run(UA_EventLoopPuffin *el, UA_UInt32 timeout)
{
    if (el->executing)
    {
    ...
    UA_LOG_TRACE(el->eventLoop.logger, UA_LOGCATEGORY_EVENTLOOP, "Iterate the Puffin EventLoop");
    ...
    processDelayed(el);
    UA_LOG_TRACE(el->eventLoop.logger, UA_LOGCATEGORY_EVENTLOOP, "Delayed callbacks are processed");
...
static UA_StatusCode UA_EventLoopPuffin_registerEventSource(UA_EventLoopPuffin *el,
                                                            UA_EventSource *es)
{
    if (!es)
    {
...
static UA_StatusCode UA_EventLoopPuffin_deregisterEventSource(UA_EventLoopPuffin *el,
                                                              UA_EventSource *es)
{
    if (es->state != UA_EVENTSOURCESTATE_STOPPED)
    {
...
static UA_DateTime UA_EventLoopPuffin_DateTime_now(UA_EventLoop *el)
{
...
static UA_DateTime UA_EventLoopPuffin_DateTime_nowMonotonic(UA_EventLoop *el)
{
    UA_EventLoopPuffin *pel = (UA_EventLoopPuffin *)el;
    struct timespec ts;
...
static UA_Int64 UA_EventLoopPuffin_DateTime_localTimeUtcOffset(UA_EventLoop *el)
{
...
static UA_StatusCode UA_EventLoopPuffin_free(UA_EventLoopPuffin *el)
{
...
static void UA_EventLoopPuffin_lock(UA_EventLoop *public_el)
{
}

static void UA_EventLoopPuffin_unlock(UA_EventLoop *public_el)
{
}

UA_EventLoop *UA_EventLoop_new_POSIX(const UA_Logger *logger)
{
    UA_EventLoopPuffin *el = (UA_EventLoopPuffin *)UA_calloc(1, sizeof(UA_EventLoopPuffin));
    if (!el)
        return NULL;
    ...
    el->eventLoop.dateTime_nowMonotonic = UA_EventLoopPuffin_DateTime_nowMonotonic;
    el->eventLoop.dateTime_localTimeUtcOffset = UA_EventLoopPuffin_DateTime_localTimeUtcOffset;
    ...
    el->eventLoop.registerEventSource =
        (UA_StatusCode(*)(UA_EventLoop *, UA_EventSource *))UA_EventLoopPuffin_registerEventSource;
    el->eventLoop.deregisterEventSource =
        (UA_StatusCode(*)(UA_EventLoop *,
                          UA_EventSource *))UA_EventLoopPuffin_deregisterEventSource;
                                                    ...
                                                    size_t bufSize)
{
    ...
    if (pcm->txBuffer_index > 0)
    {
        ...
        if (buf->length < bufSize)
            return UA_STATUSCODE_BADOUTOFMEMORY;
        UA_LOG_TRACE(pcm->cm.eventSource.eventLoop->logger,
                     UA_LOGCATEGORY_NETWORK,
                                          ...
                                          UA_ByteString *buf)
{
    ...
    {
        if (buf != &pcm->txBuffer)
        { // /!\ Memory leak if &txBuffer == buf
            UA_LOG_DEBUG(pcm->cm.eventSource.eventLoop->logger,
                         UA_LOGCATEGORY_NETWORK,
                         "TCP\t| Reinit of buffer at %p, of size %lu",
                         buf->data,
                         buf->length);
...
UA_StatusCode UA_EventLoopPuffin_allocateStaticBuffers(UA_PuffinConnectionManager *pcm)
{
    ...
    const UA_UInt32 *configRxBufSize =
        (const UA_UInt32 *)UA_KeyValueMap_getScalar(&pcm->cm.eventSource.params,
