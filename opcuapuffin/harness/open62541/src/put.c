...
static const OPCUA_PUT_INTERFACE open62541_vtable = {
    ...
    .agent_interface = {.destroy = open62541_destroy,
...
UA_LogLevel open62541_log_level(RustLogFilter level)
{
    ...
    if (level == RUST_LOG_INFO)
        return UA_LOGLEVEL_INFO;
    if (level == RUST_LOG_DEBUG)
...
AGENT open62541_create(const APPLICATION_DESCRIPTOR *descriptor)
{
    ...
    if (descriptor->role == CLIENT)
    {
        ...
        /* Exchange the logger */
        UA_Logger logger = UA_Log_Stdout_withLevel(open62541_log_level(descriptor->log_level));
...
void open62541_destroy(AGENT agent)
{
    ...
    if (agent->role == SERVER)
    {
        ...
        if (status)
            _log(PUFFIN.error, "UA Server shutdown returned %s", UA_StatusCode_name(status));
         ...
         *  2) the custom Puffin event loop: its TCP connection source only reaches STOPPED once
         * every socket is closed (TCP_checkStopped: fdsSize == 0), and connections close over later
         *     run() cycles. Once the *server* is STOPPED, UA_Server_run_iterate no longer drives
         * the event loop, so we must pump the event loop itself until it (and its sources) STOP. */
        for (int i = 0;
             i < 1000 && UA_Server_getLifecycleState(server) != UA_LIFECYCLESTATE_STOPPED;
             i++)
        {
        ...
        if (UA_Server_getLifecycleState(server) != UA_LIFECYCLESTATE_STOPPED)
            _log(PUFFIN.error,
                 "UA Server did not reach STOPPED before delete (state=%d)",
                 (int)UA_Server_getLifecycleState(server));
        if (el && el->state != UA_EVENTLOOPSTATE_STOPPED)
            _log(PUFFIN.error,
                 "EventLoop did not reach STOPPED before delete (state=%d)",
                 (int)el->state);
        _log(PUFFIN.trace, "Server delete ...");
        status = UA_Server_delete(server);
        if (status)
            _log(PUFFIN.error, "UA Server delete returned %s", UA_StatusCode_name(status));
    }
...
RESULT open62541_progress(AGENT agent)
{
    ...
    return PUFFIN.make_result(RESULT_ERROR_OTHER,
                              "Agent role unimplemented in function 'progress'");
}
