...
#define ZIP_FUNCTIONS(name, type, field, keytype, keyfield, cmp)                                   \
                                                                                                   \
    ZIP_UNUSED static ZIP_INLINE void name##_ZIP_INSERT(struct name *head, struct type *el)        \
    {                                                                                              \
        __ZIP_INSERT(head,                                                                         \
                     (zip_cmp_cb)cmp,                                                              \
                     ...
                     el);                                                                          \
                                                                                                   ...
                                                                                                   \
    ZIP_UNUSED static ZIP_INLINE struct type *name##_ZIP_REMOVE(struct name *head,                 \
                                                                struct type *elm)                  \
    {                                                                                              \
        return (struct type *)__ZIP_REMOVE(head,                                                   \
                                           (zip_cmp_cb)cmp,                                        \
                                           offsetof(struct type, field),                           \
                                           offsetof(struct type, keyfield),                        \
                                           elm);                                                   \
                                                                                                   ...
                                                                                                   \
    ZIP_UNUSED static ZIP_INLINE struct type *name##_ZIP_FIND(struct name *head,                   \
                                                              const keytype *key)                  \
    {                                                                                              \
        ...
        return cur;                                                                                \
                                                                                                   ...
                                                                                                   \
    ZIP_UNUSED static ZIP_INLINE struct type *name##_ZIP_MIN(struct name *head)                    \
    {                                                                                              \
        struct type *cur = ZIP_ROOT(head);                                                         \
        if (!cur)                                                                                  \
            return NULL;                                                                           \
        ...
        return cur;                                                                                \
                                                                                                   ...
                                                                                                   \
    ZIP_UNUSED static ZIP_INLINE struct type *name##_ZIP_MAX(struct name *head)                    \
    {                                                                                              \
        struct type *cur = ZIP_ROOT(head);                                                         \
        if (!cur)                                                                                  \
            return NULL;                                                                           \
    ...
    typedef void *(*name##_cb)(void *context, struct type *elm);                                   \
                                                                                                   \
    ZIP_UNUSED static ZIP_INLINE void *name##_ZIP_ITER(struct name *head,                          \
                                                       name##_cb cb,                               \
                                                       void *context)                              \
    {                                                                                              \
        return __ZIP_ITER(offsetof(struct type, field), (zip_iter_cb)cb, context, ZIP_ROOT(head)); \
    }                                                                                              \
                                                                                                   \
    ZIP_UNUSED static ZIP_INLINE void *name##_ZIP_ITER_KEY(struct name *head,                      \
                                                           const keytype *key,                     \
                                                           name##_cb cb,                           \
                                                           void *context)                          \
    {                                                                                              \
        return __ZIP_ITER_KEY((zip_cmp_cb)cmp,                                                     \
                              offsetof(struct type, field),                                        \
                              offsetof(struct type, keyfield),                                     \
                              key,                                                                 \
                              (zip_iter_cb)cb,                                                     \
                              ...
                              ZIP_ROOT(head));                                                     \
                                                                                                   ...
                                                                                                   \
    ZIP_UNUSED static ZIP_INLINE struct type *name##_ZIP_ZIP(struct type *left,                    \
                                                             struct type *right)                   \
    {                                                                                              \
        return (struct type *)__ZIP_ZIP(offsetof(struct type, field), left, right);                \
    }                                                                                              \
                                                                                                   \
    ZIP_UNUSED static ZIP_INLINE void name##_ZIP_UNZIP(struct name *head,                          \
                                                       const keytype *key,                         \
                                                       struct name *left,                          \
                                                       struct name *right)                         \
    {                                                                                              \
        __ZIP_UNZIP((zip_cmp_cb)cmp,                                                               \
                    offsetof(struct type, field),                                                  \
                    offsetof(struct type, keyfield),                                               \
                    key,                                                                           \
                    head,                                                                          \
