...
#[derive(Clone)]
pub struct CertificatePrivateKey(pub Vec<u8>);

pub fn fn_bob_cert() -> Result<Certificate, FnError> {
    Ok(Certificate(BOB_CERT.1.into()))
}

pub fn fn_bob_key() -> Result<CertificatePrivateKey, FnError> {
    Ok(CertificatePrivateKey(BOB_PRIVATE_KEY.1.into()))
}

pub fn fn_alice_cert() -> Result<Certificate, FnError> {
    Ok(Certificate(ALICE_CERT.1.into()))
}

pub fn fn_alice_key() -> Result<CertificatePrivateKey, FnError> {
    Ok(CertificatePrivateKey(ALICE_PRIVATE_KEY.1.into()))
}

pub fn fn_eve_cert() -> Result<Certificate, FnError> {
    Ok(Certificate(EVE_CERT.1.into()))
}

pub fn fn_random_ec_cert() -> Result<Certificate, FnError> {
    Ok(Certificate(RANDOM_EC_CERT.1.into()))
}

pub fn fn_certificate_entry(cert: &Certificate) -> Result<CertificateEntry, FnError> {
    Ok(CertificateEntry {
        cert: cert.clone(),
...
pub fn fn_eve_pkcs1_signature() -> Result<Signature, FnError> {
    Ok(Signature(vec![
...
fn _fn_rsa_sign(
    ...
    let key = RsaKeyPair::from_der(&private_key.0)
...
fn _fn_ecdsa_sign(
    ...
    let key = EcdsaSigningKey::new(
        &PrivateKey(private_key.0.to_vec()),
