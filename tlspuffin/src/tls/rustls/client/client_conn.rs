...
use crate::tls::rustls::{
    ...
    conn::{CommonState, ConnectionCommon, Protocol, Side},
    error::Error,
    kx::SupportedKxGroup,
    msgs::{
...
/// # use crate::tls::rustls::tls::rustls::ServerName;
...
#[doc(hidden)]
impl<'a> TryFrom<&'a mut crate::tls::rustls::Connection> for &'a mut ClientConnection {
    type Error = ();

    fn try_from(value: &'a mut crate::tls::rustls::Connection) -> Result<Self, Self::Error> {
        use crate::tls::rustls::Connection::*;
        match value {
            Client(conn) => Ok(conn),
            Server(_) => Err(()),
        }
...
impl From<ClientConnection> for crate::tls::rustls::Connection {
...
impl crate::tls::rustls::conn::SideData for ClientConnectionData {}
