...
enum_builder! {
    ...
    // `Unknown(u8)` is left out: no function symbol produces a `u8`.
...
enum_builder! {
    ...
    // `Unknown(u8)` is left out: no function symbol produces a `u8`. `HelloRetryRequest` is
    // left out because `HandshakeMessagePayload::encode` writes it as `ServerHello`, so the
    // symbol's own encoding would not match its position in the encoded message.
    EnumUnknown { #[constructor_skip] }
    EnumVal{
        HelloRequest => 0x00,
        ClientHello => 0x01,
        ...
        EndOfEarlyData => 0x05,
        #[constructor_skip] HelloRetryRequest => 0x06,
...
enum_builder! {
    ...
    @U16
    EnumName: NamedGroup;
    // `ALL_KX_GROUPS` (see `rustls::kx`) implements the key exchange for X25519, secp256r1 and
    // secp384r1 only. A `NamedGroup` that none of them matches makes every symbol computing a key
    // exchange (`fn_new_pubkey12`, `fn_encrypt12`, `fn_derive_psk`, the transcripts, ...) fail to
    // evaluate, so the message is never even built — registering all ten variants left 7 of the 10
    // choices unable to produce anything. Two unsupported groups stay registered so the case
    // remains reachable: `secp521r1`, a real curve we happen not to implement, and `FFDHE2048`, one
    // representative of the finite-field family (which a PUT parses down a different path from the
    // EC ones). `Unknown(u16)` is skipped too — bit-level mutations already reach arbitrary group
    // codes, and it needs a `u16` argument that only makes the term deeper.
    EnumUnknown { #[constructor_skip] }
    EnumVal{
        secp256r1 => 0x0017,
        secp384r1 => 0x0018,
        secp521r1 => 0x0019,
        X25519 => 0x001d,
        #[constructor_skip] X448 => 0x001e,
        FFDHE2048 => 0x0100,
        #[constructor_skip] FFDHE3072 => 0x0101,
        #[constructor_skip] FFDHE4096 => 0x0102,
        #[constructor_skip] FFDHE6144 => 0x0103,
        #[constructor_skip] FFDHE8192 => 0x0104
    }
}
...
enum_builder! {
    ...
    #[derive(Constructor)]
    #[constructor(TLS_SIGNATURE, TLSProtocolTypes)]
    // The `Vec<CipherSuite>` constructors plus a curated handful of values: a symbol per variant
    // would put 377 cipher suites in the signature, of which `suite_as_supported_suite` implements
    // a few, so a randomly generated `CipherSuite` would almost never let a crypto function symbol
    // (`fn_encrypt12`, `fn_encrypt_application`, `fn_client_sign_transcript`, ...) evaluate.
    // The variants below carrying `#[constructor_no_skip]` are the ones the fuzzer may build; each
    // note says what it is there for. To add one, mark it here — there is no longer a hand-written
    // counterpart in `fn_fields` to keep in step.
    #[constructor_list(no_codec_impl)]
    ...
    EnumVal{
        ...
        // export-grade, for downgrade attempts
        #[constructor_no_skip]
        ...
        // `suite_as_supported_suite` has no implementation for it
        #[constructor_no_skip]
        ...
        // the TLS 1.2 suite the seeds negotiate
        #[constructor_no_skip]
        ...
        // valid but outside the list configured on the PUT
        #[constructor_no_skip]
