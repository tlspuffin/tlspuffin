...
/// has the given $payload_type.  If not, return Err(crate::tls::rustls::tls::rustls::Error) quoting
...
macro_rules! require_handshake_msg(
  ( $m:expr, $handshake_type:path, $payload_type:path ) => (
    match &$m.payload {
        MessagePayload::Handshake($crate::tls::rustls::msgs::handshake::HandshakeMessagePayload {
            payload: $payload_type(hm),
            ..
        }) => Ok(hm),
        payload => Err($crate::tls::rustls::check::inappropriate_handshake_message(
            payload,
            &[$crate::tls::rustls::msgs::enums::ContentType::Handshake],
            &[$handshake_type]))
    }
  )
);
...
macro_rules! require_handshake_msg_move(
  ( $m:expr, $handshake_type:path, $payload_type:path ) => (
    match $m.payload {
        ...
        payload =>
            Err($crate::tls::rustls::check::inappropriate_handshake_message(
                &payload,
                &[$crate::tls::rustls::msgs::enums::ContentType::Handshake],
                &[$handshake_type]))
    }
...
pub fn inappropriate_message(payload: &MessagePayload, content_types: &[ContentType]) -> Error {
    warn!(
