...
use super::{
    hs::{self, HandshakeHashOrBuffer, ServerContext},
    server_conn::ServerConnectionData,
};
#[cfg(feature = "logging")]
...
use crate::tls::rustls::{
    ...
    msgs::{
        codec::Codec,
        enums::{AlertDescription, ContentType, HandshakeType, KeyUpdateRequest, ProtocolVersion},
        handshake::{
    ...
    ticketer,
    tls13::{
        key_schedule::{KeyScheduleTraffic, KeyScheduleTrafficWithClientFinishedPending},
        Tls13CipherSuite,
    },
    verify,
};
#[cfg(feature = "quic")]
use crate::tls::rustls::{check::inappropriate_message, conn::Protocol};

mod client_hello {
    use super::*;
    #[cfg(feature = "quic")]
    use crate::tls::rustls::quic;
    use crate::tls::rustls::{
        kx,
        msgs::{
            base::{Payload, PayloadU8},
            ccs::ChangeCipherSpecPayload,
            enums::{Compression, NamedGroup, PSKKeyExchangeMode, SignatureScheme},
            handshake::{
                ...
                DigitallySignedStruct, HelloRetryExtension, HelloRetryRequest, KeyShareEntry,
                Random, ServerExtension, ServerHelloPayload, SessionID,
            },
        },
        server::common::ActiveCertifiedKey,
        sign,
        tls13::key_schedule::{KeyScheduleEarly, KeyScheduleHandshake, KeySchedulePreHandshake},
    };
    ...
    pub(in crate::tls::rustls::server) struct CompleteClientHelloHandling {
        pub(in crate::tls::rustls::server) config: Arc<ServerConfig>,
        pub(in crate::tls::rustls::server) transcript: HandshakeHash,
        pub(in crate::tls::rustls::server) suite: &'static Tls13CipherSuite,
        pub(in crate::tls::rustls::server) randoms: ConnectionRandoms,
        pub(in crate::tls::rustls::server) done_retry: bool,
        pub(in crate::tls::rustls::server) send_ticket: bool,
        pub(in crate::tls::rustls::server) extra_exts: Vec<ServerExtension>,
    }
    ...
    impl CompleteClientHelloHandling {
        ...
        pub(in crate::tls::rustls::server) fn handle_client_hello(
            mut self,
