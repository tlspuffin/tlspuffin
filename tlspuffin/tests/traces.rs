...
pub fn server_attacker_with_hello_retry_request(client: AgentName) -> Trace<TLSProtocolTypes> {
    let curve = term! {
    ...
    let server_hello_transcript = term! {
        fn_append_transcript(
            (fn_append_transcript(
                (fn_append_transcript(
                    // Compute the hash of the first Client Hello using the HRR cipher (SHA384),
                    // then start a fresh SHA256 context (final cipher) seeded with that hash.
                    // This replicates wolfssl's parallel hash behavior: SHA384(CH1) is wrapped in
                    // a message_hash (0xfe), then fed into hashSha256 for key derivation.
                    // see RFC 8446 section 4.4.1
                    (fn_new_hrr_transcript_split(
                        ((client, 0)[Some(TlsQueryMatcher::Handshake(Some(HandshakeType::ClientHello)))]),
                        fn_cipher_suite13_aes_256_gcm_sha384, // HRR cipher suite (SHA384) for CH1 rollup
                        fn_cipher_suite13_aes_128_gcm_sha256  // final cipher suite (SHA256) for key derivation
                    )),
                    (@server_hrr)
                )),
                ((client, 1)[Some(TlsQueryMatcher::Handshake(Some(HandshakeType::ClientHello)))])
