...
// Defined in arc4random_prng.c (linked from the LibreSSL vendor library).
// LibreSSL uses arc4random internally, so we must reseed it too.
extern void put_rng_reseed(const uint8_t *buffer, size_t length);
...
void rng_reseed(const uint8_t *buffer, size_t length)
{
    if (buffer == NULL || length < sizeof(uint64_t))
    {
        ...
        put_rng_reseed(NULL, 0);
        return;
    }

    seed = *((uint64_t *)buffer);
    put_rng_reseed(buffer, length);
}
