...
static void libssh_destroy(AGENT agent)
{
    ...
    /* ssh_free() closes put_fd in most cases, but not when the session
       ...
       still open before closing to avoid a double-close.

       KNOWN LIMITATION (accepted; do not "fix" without ASAN A/B evidence):
       this fcntl probe has a latent fd-number-reuse hazard — if ssh_free()
       closed put_fd's number and it was recycled before this check, we could
       close the wrong fd. It is a single-threaded harness (not a true TOCTOU)
       and has never fired across the multi-million-exec campaigns (0 ASAN
       findings). The "proper" ownership rewrites (set put_fd=-1 on transfer, or
       dup the fd) each trade this never-observed hazard for a WORSE one: an fd
       LEAK in exactly the "session errored before socket init" case this guard
       exists for (libssh does not close put_fd then), or a teardown/EOF-timing
       change that can perturb the 0-diff corpus. The guard is kept deliberately. */
