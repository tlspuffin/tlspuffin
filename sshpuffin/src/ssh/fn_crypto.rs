...
pub fn fn_decrypt_packet_aesgcm(
    ...
    let msg = SshMessage::read(&mut reader)
        .ok_or_else(|| FnError::Malformed("failed to parse decrypted SshMessage".into()))?;
    // The stripped payload must be consumed in full: if the message model decodes
    // fewer bytes than the payload holds, the trailing bytes are a real framing
    // divergence (or an incomplete message model), not something to silently drop.
    if reader.left() != 0 {
        return Err(FnError::Malformed(
            "trailing bytes after the decrypted SshMessage in the SSH payload".into(),
        ));
    }
    Ok(msg)
}
