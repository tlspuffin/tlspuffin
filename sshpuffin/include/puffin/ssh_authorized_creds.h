...
static const SshAuthorizedKey SSH_AUTHORIZED_KEYS[] = {
    ...
    {"user", {0xa4, 0x9e, 0xba, 0xc7, 0x76, 0xcc, 0x3a, 0x80, 0x37, 0x2c, 0x44,
    ...
    {"userb", {0x23, 0x8c, 0x19, 0xea, 0x25, 0xb8, 0xb9, 0xc1, 0xec, 0x15, 0xbc,
...
static inline bool ssh_creds_key_authorized(const char *user, const uint8_t *fp, size_t fp_len)
{
    if (user == NULL || fp == NULL || fp_len != 32)
        return false;
    for (size_t i = 0; i < sizeof(SSH_AUTHORIZED_KEYS) / sizeof(SSH_AUTHORIZED_KEYS[0]); i++)
    {
...
static inline bool ssh_creds_password_authorized(const char *user, const uint8_t *pw, size_t pw_len)
{
    if (user == NULL || pw == NULL)
        return false;
    for (size_t i = 0; i < sizeof(SSH_AUTHORIZED_PW) / sizeof(SSH_AUTHORIZED_PW[0]); i++)
    {
