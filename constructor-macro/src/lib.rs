...
///   `fn_list_*_empty` and `fn_list_*_append`; see [`LIST_ATTR`] for its flags.
/// * `#[constructor_skip]` — on an enum variant. Generates nothing at all for that variant; see
///   [`SKIP_ATTR`].
/// * `#[constructor_skip_all]` — on an enum. Skips every variant, so only those carrying
///   `#[constructor_no_skip]` are generated; see [`SKIP_ALL_ATTR`].
/// * `#[constructor_no_skip]` — on an enum variant. Keeps that variant under
///   `#[constructor_skip_all]`; see [`NO_SKIP_ATTR`].
#[proc_macro_derive(
    Constructor,
    attributes(
        constructor,
...
pub fn constructor_macro(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    map_type_definition(&input.ident, &input.generics, &input, &options).into()
}
