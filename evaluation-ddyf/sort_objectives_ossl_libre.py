...
buckets: dict[str, BucketCondition] = {
    ...
    # -------------------------------------------------------------------------
    # [BENIGN] libre_finished_claim_psk_codec_fail  (1 trace)
    #
    # Same LibreSSL root cause as libre_finished_claim_silent_ossl — the
    # tls13_handshake_recv_action msg_callback in tls13_handshake.c fires before
    # the message-type guard (line 531 before line 540), causing LibreSSL's TLS
    # 1.3 server (agent.1) to emit a spurious Finished claim even when the
    # handshake never meaningfully proceeds.
    #
    # Triggering path: the attacker sends fn_fill_binder(fn_client_hello(...),
    # fn_derive_psk(...)) where fn_derive_psk references transcripts from
    # AgentName(0) that does not exist in this trace. Both PUTs fail with
    # FnError::Codec("Failed to parse server ecdh public key") at step 0
    # evaluation; neither PUT completes the handshake. LibreSSL's server
    # nonetheless emits a Finished claim and HandshakeMessagePayload extra
    # knowledges; OpenSSL emits neither.
    #
    # No security impact: the session is entirely invalid (Codec failure,
    # no ECDH key derivation, no application data); CVSS 0.0.
    #
    # C1: Same root cause as libre_finished_claim_silent_ossl (callback before
    #     type guard in tls13_handshake.c), confirmed by same claim polarity.
    # C2: DifferentClaimC("()", "Finished") + NotC(AlertMessagePayload) +
    #     TermContainsC("fn_fill_binder") — three independent anchors that
    #     together uniquely identify this PSK-binder-codec-fail trigger path.
    #     Excludes existing bucket (which requires AlertMessagePayload);
    #     fn_fill_binder is in the Input step recipe (steps #0 and #1).
    # C3: 1/1 traces (100%).
    # C4: Only 1 trace — below the typical 3-trace threshold. Root cause is
    #     structurally identical to the 2128-trace libre_finished_claim_silent_ossl
    #     bucket (same LibreSSL code path, same CVSS 0.0 outcome); the single
    #     trace represents the PSK-binder-Codec edge case of that defect.
    #     No statistical verification possible; listed as singleton.
    # -------------------------------------------------------------------------
    # AUDITED
    "libre_finished_claim_psk_codec_fail/": AllC(
        DifferentClaimC(in_first_type="()", in_second_type="Finished"),
        NotC(KnowledgeContainsC(OSSL, "AlertMessagePayload")),
    ...
    # Bug report: BUGS/libressl_silent_alert_family.md (row 4) — subsumes BUGS/libressl_server_unexpected_msg_silent.md
    ...
    # Bug report: BUGS/libressl_silent_alert_family.md (rows 1-3) — subsumes BUGS/libressl_silent_abort_negotiation_failure.md
    ...
    # Bug report: BUGS/libressl_silent_alert_family.md (row 5) — subsumes BUGS/ossl_alert_silent_hrr_key_share_length_mismatch.md
    ...
    # Bug report: BUGS/libressl_silent_alert_family.md (row 6) — subsumes BUGS/ossl_alert_silent_hrr_key_share_bad_group.md
    ...
    # -------------------------------------------------------------------------
    # AUDITED BY GEMINI: APPROVED. RFC 8446 §6 violation (Silent Abort).
    # Bug report: BUGS/libressl_silent_alert_family.md (row 7) — subsumes BUGS/ossl_alert_silent_hrr_no_change.md
    ...
    # (ossl_alert_silent_client_misc was split into 8 sub-buckets in the
    # INCREMENTAL PASS 2026-05-21 section further below, after Auditor revision.)
    ...
    # -------------------------------------------------------------------------
    # [BENIGN] ossl_alert_libre_proceeds_bad_extension  (463 traces) — RECLASSIFIED 2026-05-24
    #
    # Re-audit 2026-05-24: all 463 traces have ClientHellos with duplicate extension
    # types (verified empirically). The top duplicated types are SCT (RFC 6962 type 18),
    # signature_algorithms_cert (type 50), extended_master_secret (type 23), and
    # early_data (type 42) — all OUTSIDE LibreSSL's 15-entry tls_extensions[] table
    # (ssl_tlsext.c:2118-2335). LibreSSL's tlsext_parse silently skips unknown
    # extensions per RFC 5246 §7.4.1.4 / RFC 8446 §4.2 ("continue;" at ssl_tlsext.c:
    # 2569-2571), so unknown duplicates never reach the duplicate check at lines
    # 2587-2590. OpenSSL's tls_collect_extensions knows >40 extensions and rejects
    # duplicates of all of them at extensions.c:651. Strictness divergence, not RFC
    # violation. No security impact (0/463 Finished).
    # (Earlier "premature handshake commitment" RFC 8446 §9.2 framing 2026-05-22 and
    # "First-Match version bug → TLS 1.2 fall-through" framing 2026-05-23 both
    # superseded; see §10 of the bug report.)
    # -------------------------------------------------------------------------
    # AUDITED — RECLASSIFIED BENIGN 2026-05-24
    # Bug report: BUGS/libressl_proceeds_bad_extension.md
    "ossl_alert_libre_proceeds_bad_extension/": AllC(
        KnowledgeDiffC(
            first_type_name="tlspuffin::tls::rustls::msgs::alert::AlertMessagePayload",
            second_type_name="tlspuffin::tls::rustls::msgs::handshake::HandshakeMessagePayload",
        ),
        StatusC(OSSL, "tls_collect_extensions", first_to_fail=False),
    ),

    # =========================================================================
    # INCREMENTAL PASS 2026-05-21 — new patterns from 6,154 new traces
    # (all revised after Auditor Audit-1 appendix 2026-05-21)
    # =========================================================================

    # -------------------------------------------------------------------------
    # [BENIGN] harness_decrypt_key_mismatch  (~1510 traces)
    #
    # Root cause: harness artifact — NOT a real LibreSSL behavioral divergence.
    # The harness fn_decrypt_handshake_flight_with_secret preprocessing step uses
    # OpenSSL's handshake secret to attempt to decrypt LibreSSL's server flight.
    # Because both PUTs use different ephemeral ECDH server keys per run, the
    # derived handshake secrets differ; decryption of LibreSSL's ciphertext with
    # OpenSSL's secret fails. The HandshakeMessagePayload != () differences
    # (EncryptedExtensions, CertificateRequest, Certificate, CertificateVerify,
    # Finished) are harness artifacts. LibreSSL also emits ChangeCipherSpec
    # alongside its HRR/server flight (TLS 1.3 compat mode, RFC 8446 §D.4).
    # Both PUTs complete the protocol successfully; no alert, no key leak.
    # Anchor: fn_encrypt_handshake appears in Input step 2 of these TLS 1.3
    # mutual-auth traces (attacker sends encrypted Finished/Certificate as client).
    #
    # C1: Single root cause — harness cross-run decryption key mismatch
    # C2: KnowledgeDiffC(Handshake,()) anchored by TermContainsC(fn_encrypt_handshake)
    # C3: ~1509/1510 traces (99.9%)
    # C4: Survey agent analyzed 2 traces; root cause identical in both
    # -------------------------------------------------------------------------
    # AUDITED
    # Anchor: two complementary trace-recipe terms cover both subsets:
    #   fn_encrypt_handshake — mutual-auth traces (attacker sends encrypted
    #                           Certificate/Finished as TLS 1.3 client)
    #   fn_key_share_extension_new — non-mutual-auth HRR traces (attacker sends
    #                                ClientHello with empty key_share, triggering
    #                                HRR; the empty extension creates the key mismatch)
    "harness_decrypt_key_mismatch/": AllC(
        KnowledgeDiffC(
            first_type_name="tlspuffin::tls::rustls::msgs::handshake::HandshakeMessagePayload",
            second_type_name="()",
        ),
        AnyC(
            TermContainsC(OSSL, "fn_encrypt_handshake"),
            TermContainsC(OSSL, "fn_key_share_extension_new"),
        ),
    ),

    # -------------------------------------------------------------------------
    # [BENIGN] libre_sni_ack_divergence  (~2581 traces: B1=1252, B2=1061, B3=136, B4=68, B5=64)
    #
    # Root cause: LibreSSL server echoes the SNI acknowledgement (ServerNameAck /
    # server_name extension, zero-length body) in ServerHello (TLS 1.2, RFC 6066
    # §3.1 SHOULD) or EncryptedExtensions (TLS 1.3, RFC 8446 §4.3.1) when the
    # ClientHello includes a server_name extension. OpenSSL omits the echo. Both
    # are RFC-compliant: RFC 6066 §3.1 SHOULD, not MUST. Extension ordering
    # variation (B1–B5) is positional artifact of LibreSSL placing ServerNameAck
    # first; B4 (TLS 1.3) shows the same root cause via EncryptedExtensions.
    #
    # C1: Single root cause — LibreSSL SNI echo; OpenSSL omits
    # C2: InnerKnowledgeC("ServerNameAck") anchored by BothServerHello/BothEncryptedExtensions
    #     context — confirms InnerDifference is within a ServerHello or EE, not
    #     an unrelated context; polarity is confirmed by the diff format
    #     "Added(0, ServerNameAck)" / "Changed(0, ..., ServerNameAck)"
    # C3: ~2579/2581 traces (99.9%)
    # C4: Survey agent analyzed 5 traces per sub-pattern (B1–B5)
    # -------------------------------------------------------------------------
    # AUDITED
    "libre_sni_ack_divergence/": AllC(
        InnerKnowledgeC("ServerNameAck", "HandshakeMessagePayload"),
        AnyC(
            InnerKnowledgeC("BothServerHello", "HandshakeMessagePayload"),
            InnerKnowledgeC("BothEncryptedExtensions", "HandshakeMessagePayload"),
        ),
    ),

    # -------------------------------------------------------------------------
    # [BENIGN] hrr_group_selection_divergence  (~371 traces)
    #
    # Root cause: Both PUTs send valid HRR when ClientHello key_share is empty.
    # OpenSSL picks P-256 (secp256r1, its server-priority); LibreSSL picks P-384
    # (secp384r1, honouring client preference). RFC 8446 §4.1.1 SHOULD not MUST.
    # Both selections are valid; client offered both groups. Handshake continues.
    #
    # C1: Single root cause — server group-priority policy divergence in HRR
    # C2: Full BothHelloRetryRequest + BothKeyShare(Different(secp256r1,secp384r1))
    #     InnerKnowledgeC — includes HRR structural context and the exact group
    #     pair; the "Different(secp256r1, secp384r1)" encodes polarity (OSSL→P256,
    #     LIBRE→P384)
    # C3: 371/371 traces (100%)
    # C4: Survey agent analyzed 2 traces; both confirmed same root cause
    # -------------------------------------------------------------------------
    # AUDITED
    "hrr_group_selection_divergence/": InnerKnowledgeC(
        "BothHelloRetryRequest([SortedExtensions(HelloRetryExtensionsChange([Changed(1, BothKeyShare(Different(secp256r1, secp384r1)))",
        "HandshakeMessagePayload",
    ),

    # =========================================================================
    # INCREMENTAL 2026-05-21 — ossl_alert_silent_* SPLIT (Auditor revision)
    # Split from ossl_alert_silent_client_misc (1047 traces) into 8 sub-buckets.
    # Base pattern: OpenSSL client detects malformed server message, alerts;
    # LibreSSL client produces no alert — RFC 8446 §6 / RFC 5246 §7.2 violation.
    # All sub-buckets share: KnowledgeDiffC(Alert,()), no Finished claim diff,
    # no D1 contamination, specific StatusC anchor grounded in OpenSSL code path.
    # =========================================================================

    # -------------------------------------------------------------------------
    # [RFC] ossl_alert_silent_client_cert_req_length  (~579 traces)
    # OpenSSL client rejects a malformed CertificateRequest: parse_ca_names
    # (statem_lib.c:2673) or tls_process_certificate_request (statem_clnt.c:2636)
    # finds a length mismatch. LibreSSL client produces no alert.
    # -------------------------------------------------------------------------
    # AUDITED
    # Bug report: BUGS/libressl_silent_alert_family.md (row 8)
    "ossl_alert_silent_client_cert_req_length/": AllC(
        KnowledgeDiffC(
            first_type_name="tlspuffin::tls::rustls::msgs::alert::AlertMessagePayload",
            second_type_name="()",
        ),
        ...
        NotC(KnowledgeDiffC(
            first_type_name="tlspuffin::tls::rustls::msgs::handshake::HandshakeMessagePayload",
            second_type_name="tlspuffin::tls::rustls::msgs::alert::AlertMessagePayload",
        )),
        AnyC(
            StatusC(OSSL, "parse_ca_names", first_to_fail=False),
            StatusC(OSSL, "tls_process_certificate_request:length mismatch", first_to_fail=False),
        ),
    ),

    # -------------------------------------------------------------------------
    # [RFC] ossl_alert_silent_excessive_msg_size  (~321 traces)
    # OpenSSL client's statem.c:649 read_state_machine guard rejects an
    # oversized server message. LibreSSL client produces no alert.
    # -------------------------------------------------------------------------
    # AUDITED
    # Bug report: BUGS/libressl_silent_alert_family.md (row 9)
    "ossl_alert_silent_excessive_msg_size/": AllC(
        KnowledgeDiffC(
            first_type_name="tlspuffin::tls::rustls::msgs::alert::AlertMessagePayload",
            second_type_name="()",
        ),
        ...
        StatusC(OSSL, "excessive message size", first_to_fail=False),
    ),

    # -------------------------------------------------------------------------
    # [RFC] ossl_alert_silent_ctos_key_share_length  (~34 traces)
    # OpenSSL server rejects a ClientHello key_share extension with a length
    # mismatch (extensions_srvr.c:658). LibreSSL produces no alert.
    # -------------------------------------------------------------------------
    # AUDITED
    # Bug report: BUGS/libressl_silent_alert_family.md (row 10)
    "ossl_alert_silent_ctos_key_share_length/": AllC(
        KnowledgeDiffC(
            first_type_name="tlspuffin::tls::rustls::msgs::alert::AlertMessagePayload",
            second_type_name="()",
        ),
    ...
    # -------------------------------------------------------------------------
    # [RFC] ossl_alert_silent_invalid_alert  (~30 traces)
    # OpenSSL rejects an invalid alert record (ssl3_read_bytes:invalid alert).
    # LibreSSL produces no alert in response.
    # -------------------------------------------------------------------------
    # AUDITED
    # Bug report: BUGS/libressl_silent_alert_family.md (row 11)
    "ossl_alert_silent_invalid_alert/": AllC(
        KnowledgeDiffC(
            first_type_name="tlspuffin::tls::rustls::msgs::alert::AlertMessagePayload",
            second_type_name="()",
        ),
        ...
        StatusC(OSSL, "invalid alert", first_to_fail=False),
    ),

    # -------------------------------------------------------------------------
    # [RFC] ossl_alert_silent_stoc_ecpoint  (~29 traces)
    # OpenSSL client rejects a bad EC point in the server's key_share extension
    # (extensions_clnt.c:1943). LibreSSL client produces no alert.
    # -------------------------------------------------------------------------
    # AUDITED
    # Bug report: BUGS/libressl_silent_alert_family.md (row 12)
    "ossl_alert_silent_stoc_ecpoint/": AllC(
        KnowledgeDiffC(
            first_type_name="tlspuffin::tls::rustls::msgs::alert::AlertMessagePayload",
            second_type_name="()",
        ),
    ...
    # -------------------------------------------------------------------------
    # [RFC] ossl_alert_silent_client_cipher_select  (~29 traces)
    # OpenSSL client rejects a ServerHello offering an unrecognised or wrong
    # cipher (set_client_ciphersuite). LibreSSL client produces no alert.
    # -------------------------------------------------------------------------
    # AUDITED
    # Bug report: BUGS/libressl_silent_alert_family.md (row 13)
    "ossl_alert_silent_client_cipher_select/": AllC(
        KnowledgeDiffC(
            first_type_name="tlspuffin::tls::rustls::msgs::alert::AlertMessagePayload",
            second_type_name="()",
        ),
        ...
        AnyC(
            StatusC(OSSL, "wrong cipher returned", first_to_fail=False),
            StatusC(OSSL, "unknown cipher returned", first_to_fail=False),
        ),
    ),

    # -------------------------------------------------------------------------
    # [RFC] ossl_alert_silent_missing_sigalgs  (~18 traces)
    # OpenSSL rejects a handshake where required signature algorithms extension
    # is absent (final_sig_algs:missing sigalgs extension). LibreSSL silent.
    # -------------------------------------------------------------------------
    # AUDITED
    # Bug report: BUGS/libressl_silent_alert_family.md (row 14)
    "ossl_alert_silent_missing_sigalgs/": AllC(
        KnowledgeDiffC(
            first_type_name="tlspuffin::tls::rustls::msgs::alert::AlertMessagePayload",
            second_type_name="()",
        ),
        ...
        StatusC(OSSL, "missing sigalgs extension", first_to_fail=False),
    ),

    # -------------------------------------------------------------------------
    # [RFC] ossl_alert_silent_client_record_too_long  (~9 traces)
    # OpenSSL rejects a TLSCiphertext record whose length exceeds RFC maximum
    # (tls_validate_record_header:data length too long, tlsany_meth.c:124).
    # LibreSSL produces no alert — distinct from ossl_record_too_long_libre_proceeds
    # (Status diff) because here the primary diff is Knowledge (Alert vs ()).
    # -------------------------------------------------------------------------
    # AUDITED
    # Bug report: BUGS/libressl_silent_alert_family.md (row 15)
    "ossl_alert_silent_client_record_too_long/": AllC(
        KnowledgeDiffC(
            first_type_name="tlspuffin::tls::rustls::msgs::alert::AlertMessagePayload",
            second_type_name="()",
        ),
        ...
        StatusC(OSSL, "data length too long", first_to_fail=False),
    ),

    # -------------------------------------------------------------------------
    # [BENIGN] libre_alert_ossl_proceeds_hrr_client  (~107 traces)
    #
    # Root cause: After receiving an HRR (containing a cookie extension or
    # magic HRR Random), OpenSSL client generates two ClientHellos (initial +
    # cookie-bearing retry per RFC 8446 §4.1.2) while LibreSSL client generates
    # only the initial ClientHello then sends IllegalParameter. Both PUTs act as
    # TLS clients; attacker controls the server. LibreSSL may be detecting a
    # specific condition in the HRR that OpenSSL accepts.
    # 103/107 traces have fn_cookie_hello_retry_extension in the trace recipe;
    # all 107 have fn_hello_retry_request_random (the HRR magic value).
    #
    # C1: HRR cookie-handling divergence; RFC compliance TBD by Auditor
    # C2: KnowledgeDiffC(Handshake,Alert) + TermContainsC(cookie or HRR magic)
    # C3: 107/107 (100%)
    # C4: Survey agent analyzed 1 D1 trace in depth
    # -------------------------------------------------------------------------
    # AUDITED
    "libre_alert_ossl_proceeds_hrr_client/": AllC(
        KnowledgeDiffC(
            first_type_name="tlspuffin::tls::rustls::msgs::handshake::HandshakeMessagePayload",
            second_type_name="tlspuffin::tls::rustls::msgs::alert::AlertMessagePayload",
        ),
        AnyC(
            TermContainsC(OSSL, "fn_cookie_hello_retry_extension"),
            TermContainsC(OSSL, "fn_hello_retry_request_random"),
        ),
    ),

    # =========================================================================
    # INCREMENTAL 2026-05-21 — reverse-polarity split (Auditor revision)
    # Split from libre_produces_more_ossl_nothing (68 traces) into 3 sub-buckets.
    # =========================================================================

    # -------------------------------------------------------------------------
    # [RFC] ossl_alert_libre_psk_kex_missing  (~53 traces)
    # OpenSSL server rejects ClientHello immediately (final_psk:missing psk
    # kex modes extension, ssl/statem/extensions.c:1793): RFC 8446 §4.2.9
    # requires psk_key_exchange_modes when offering PSK. OpenSSL alerts;
    # LibreSSL continues to emit ServerHello + Certificate (more permissive).
    # LibreSSL eventually also rejects, but after partial handshake output.
    # Polarity at knowledge[0]: Alert (OSSL) vs Handshake (LIBRE).
    # Structural note: similar to ossl_alert_libre_proceeds_bad_extension but
    # triggered via the PSK extension check rather than tls_collect_extensions.
    # -------------------------------------------------------------------------
    # AUDITED — RECLASSIFIED BENIGN 2026-05-24
    # Bug report: BUGS/libressl_proceeds_psk_kex_missing.md
    # Re-audit 2026-05-24: all 52 traces use TLS-1.2-only server config.
    # LibreSSL's tlsext_psk_server_process (ssl_tlsext.c:2023-2026) is a no-op:
    # CBS_skip(cbs, CBS_len(cbs)) — LibreSSL has no server-side TLS 1.3 PSK impl.
    # OpenSSL's final_psk (extensions.c:1788-1798) lacks a version guard; fires on
    # any pre_shared_key regardless of negotiated version, even on TLS-1.2-only servers.
    # The TLS 1.3 guard at tls13_server.c:115-118 IS correct, just structurally
    # unreachable in this corpus. Strictness divergence, not LibreSSL RFC violation.
    # (Earlier "First-Match → TLS 1.2 fall-through" linkage also REFUTED 2026-05-23.)
    "ossl_alert_libre_psk_kex_missing/": AllC(
        KnowledgeDiffC(
            first_type_name="tlspuffin::tls::rustls::msgs::alert::AlertMessagePayload",
            second_type_name="tlspuffin::tls::rustls::msgs::handshake::HandshakeMessagePayload",
        ),
        StatusC(OSSL, "final_psk", first_to_fail=False),
    ),

    # -------------------------------------------------------------------------
    # [BENIGN] libre_decrypts_ossl_mac_fail  (~12 traces)
    # LibreSSL decrypts handshake messages (HandshakeMessagePayload extra
    # knowledges) that OpenSSL cannot decrypt (OpenSSL fails with
    # tls_get_more_records:decryption failed or bad record mac).
    # LibreSSL is MORE active; OpenSSL produces no knowledge at those positions.
    # -------------------------------------------------------------------------
    # AUDITED
    "libre_decrypts_ossl_mac_fail/": AllC(
        KnowledgeDiffC(
            first_type_name="()",
            second_type_name="tlspuffin::tls::rustls::msgs::handshake::HandshakeMessagePayload",
        ),
        AnyC(
            StatusC(OSSL, "decryption failed or bad record mac", first_to_fail=False),
            StatusC(OSSL, "bad record mac", first_to_fail=False),
            StatusC(OSSL, "decryption failed", first_to_fail=False),
        ),
    ),

    # -------------------------------------------------------------------------
    # [BENIGN] libre_alerts_ossl_bad_record  (~4 traces)
    # LibreSSL sends an alert where OpenSSL produces nothing; OpenSSL fails with
    # a bad record type or decryption error. Residual reverse-polarity cases.
    # -------------------------------------------------------------------------
    # AUDITED
    "libre_alerts_ossl_bad_record/": AllC(
        KnowledgeDiffC(
            first_type_name="()",
            second_type_name="tlspuffin::tls::rustls::msgs::alert::AlertMessagePayload",
        ),
        AnyC(
            StatusC(OSSL, "bad record type", first_to_fail=False),
            StatusC(OSSL, "decryption failed", first_to_fail=False),
        ),
    ),
