...
### Step 0 — Resolve the triaging script + module  *(protocol-agnostic; do this first)*

The commands below run **as-is for any protocol** — they use `$TRIAGING_SCRIPT` (the file, for
backups) and `$TRIAGING_MODULE` (its `python -m` module path) instead of a hardcoded TLS name,
so you never copy-paste `sort_objectives_ossl_libre.py` into an SSH campaign by mistake. If the
Orchestrator already exported `TRIAGING_SCRIPT` (Launch-time precondition 5), reuse it:

```bash
# The per-protocol triaging script (glob-resolved, or set TRIAGING_SCRIPT explicitly).
...
# Module path for `python -m`: '-'->'_' (evaluation_ddyf is a symlink of evaluation-ddyf so the
# import resolves — a literal 'evaluation-ddyf' module name is invalid), '/'->'.', drop '.py'.
TRIAGING_MODULE="$(echo "$TRIAGING_SCRIPT" | sed 's,-,_,g; s,/,.,g; s,\.py$,,')"
SCRIPT_BASE="$(basename "$TRIAGING_SCRIPT" .py)"
echo "script=$TRIAGING_SCRIPT  module=$TRIAGING_MODULE"
# e.g. SSH: script=evaluation-ddyf/ssh/sort_objectives_libssh_wolfssh.py
#           module=evaluation_ddyf.ssh.sort_objectives_libssh_wolfssh
```

### Step 1 — Create the bootstrap output directory

Create it **first** — later steps `cp`/`tee` into it, so it must exist before anything else:

```bash
mkdir -p ${CAMPAIGN}/phase_0_5_bootstrap/
```

### Step 2 — Save the current bucket set if any

If you are running this on a campaign that already has a bucket set in `$TRIAGING_SCRIPT` (resumed campaign, or incremental re-run — see §"Incremental re-runs" below), back up the current script:

```bash
cp "$TRIAGING_SCRIPT" \
   "${CAMPAIGN}/phase_0_5_bootstrap/${SCRIPT_BASE}.before_bootstrap.py"
```
...
```bash
cp "$TRIAGING_SCRIPT" \
   "${CAMPAIGN}/phase_0_5_bootstrap/${SCRIPT_BASE}.after_bootstrap.py"
...
| `${CAMPAIGN}/phase_0_5_bootstrap/<script_base>.before_bootstrap.py` | Backup of pre-bootstrap script (only for resumed campaigns). `<script_base>` = the triaging script's basename, e.g. `sort_objectives_libssh_wolfssh`. |
| `${CAMPAIGN}/phase_0_5_bootstrap/<script_base>.after_bootstrap.py` | Snapshot of script with loose bootstrap buckets. |
...
| `$TRIAGING_SCRIPT` (the live triaging script) | Now contains the bootstrap buckets (tagged `# BOOTSTRAP: loose criterion`). |
