...
- **Whether the finding is a CVE candidate**. CVE-candidate inclusion (Track 2 above) has its own criteria — see `../TEMPLATES.md` (Summary-buckets) §1 Legend. A finding can fail Gate 1 (no Finished claim in the corpus) and still be a CVE candidate if criteria (i)–(iii) are met.
...
**Committed external-oracle artifact (mandatory).** Gate 0 must leave a *machine-checkable
artifact committed to the campaign folder*, not just prose — this is what makes Gate 0 a
real external oracle rather than one LLM vouching for another. Produce and commit
`${CAMPAIGN}/gate0/<finding>/` containing: (i) the exact upstream ref checked (repo + tag +
commit SHA); (ii) the fetched upstream source of the affected function (`raw.githubusercontent`
snapshot) so the affected lines are diffable; (iii) a diff (or an explicit "no patch touches
this file" note) between the vendor fork and that upstream source for the affected file; and
(iv) where an L1+ reproducer exists, its transcript. A reader must be able to re-run/inspect
the artifact and reach the same verdict without trusting the Orchestrator's or Auditor's
narrative.

**What to put in the bug report:**

- A short "Upstream-build confirmation" subsection in §4 (Root cause) or §2 (Security Gate Results) with a table of `Site / Vendor build / Upstream OPENBSD_X_Y (or equivalent tag) / Upstream master / Status`, linking the committed `gate0/` artifact. The renegotiation bug report's §4 is the worked example.
...
- **TODO (TLS owners):** the converse also holds and is not yet handled here — a genuine **TLS 1.2** session derives its key material into a *different* struct field (e.g. `master_secret_12`), not the TLS 1.3 `master_secret`, so checking only the TLS 1.3 fields for non-zero would wrongly flag a valid TLS 1.2 handshake at Gate 3. Until this gate is updated to read the version-appropriate field, treat an all-TLS-1.3-fields-zero result on a *TLS 1.2* trace as inconclusive (consult the TLS 1.2 field) rather than an automatic Gate-3 failure. (Flagged by the prompt re-audit; the real fix needs the exact `SSL`/claim struct field name and is deferred to the TLS maintainers.)
...
| **Concrete scenario constructed** | Tag `[VULN]`. Score with `CVSS_TLS.md`. Write report with `../TEMPLATES.md` (Bug report). |
