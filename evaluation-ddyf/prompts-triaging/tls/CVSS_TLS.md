...
The **multi-framing analysis is mandatory** for any finding being considered for Track 2 (CVE candidate) promotion per `SECURITY_GATE_TLS.md` Track 2 criteria. Track 1 (strict `[VULN]`) findings have unconditional exploitability by definition and may give a single CVSS.
...
This trap is about **scoring**, not about thinking. Chained-bug analysis is encouraged — record it under "Speculative attack paths" (see `SECURITY_GATE_TLS.md`). The trap is only about assigning a CVSS *score* to a chain.
