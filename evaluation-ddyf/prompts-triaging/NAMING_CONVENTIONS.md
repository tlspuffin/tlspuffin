...
where `<put1>` and `<put2>` are short PUT identifiers and `MM-DD` is the campaign start date.
Shorten only a redundant *build/patch* suffix, and **keep any digits that carry the version /
implementation identity** — otherwise two versions of the same stack collapse to the same name.
...
- **The bucket *name* is not the category.** A bucket named `benign_*` (a common naming family for known/harness-artefact classes) is **not** automatically BENIGN — its category is whatever section of `SUMMARY_BUCKETS.md` it lands in. A `benign_harness_*` bucket whose root cause is a defect in the fuzzer/harness itself is category **Bug (non-RFC)**, §3 — see `TEMPLATES.md` § "When the finding is in the fuzzer/harness itself". Do not file it as BENIGN just because of the `benign_` prefix. (This is the same "SUMMARY category is canonical" rule as above, applied to the name.)
