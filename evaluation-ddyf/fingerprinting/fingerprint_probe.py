...
def run_probe(binary, trace_path, ip, port, timeout, retries) -> dict:
...
def node_signature(binary, trace_path, ip, port, timeout, retries, repeat, tcp_mode) -> dict:
    ...
    for _ in range(max(1, repeat)):
        res = run_probe(binary, trace_path, ip, port, timeout, retries)
        if res["outcome"] != "ok":
            last_fail = res
            continue
...
def walk_tree(node, ctx, sig_len, depth=0, path=None, defaulted=0):
    ...
    res = node_signature(ctx["binary"], trace, ctx["ip"], ctx["port"],
...
def main():
    ...
    ctx_base = {"binary": binary, "ip": ip, "port": port, "timeout": args.timeout,
    ...
    result.update({"target": host, "resolved_ip": ip, "ip_family": fam, "port": port})
